...
Write-Host "  5A. npm run build" -ForegroundColor Green
Write-Host "  6A. pm2 start npm --name h2-hot2 -- start" -ForegroundColor Green
Write-Host "  7A. pm2 save" -ForegroundColor Green
Write-Host "  8A. rm -rf /www/server/nginx/proxy_cache_dir/* && nginx -s reload" -ForegroundColor Green
Write-Host ""
...
Write-Host "  6B. npm run build" -ForegroundColor Magenta
Write-Host "  7B. pm2 start h2-hot2" -ForegroundColor Magenta
Write-Host "  8B. npm run db:migrate:status     # 验证 _prisma_migrations 状态" -ForegroundColor Magenta
Write-Host "  9B. rm -rf /www/server/nginx/proxy_cache_dir/* && nginx -s reload" -ForegroundColor Magenta
...
Write-Host "  4C. npm run build" -ForegroundColor Cyan
Write-Host "  5C. pm2 start h2-hot2" -ForegroundColor Cyan
Write-Host "  6C. rm -rf /www/server/nginx/proxy_cache_dir/* && nginx -s reload" -ForegroundColor Cyan
Write-Host "  7C. 可选: npm run db:cleanup-logs # 清理到期日志,不删除未完成投递事实" -ForegroundColor Cyan
