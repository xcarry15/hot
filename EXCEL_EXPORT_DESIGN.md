...
| Sheet | 内容 | 主键/关联字段 |
| --- | --- | --- |
| `ExportMeta` | 导出版本、生成时间、快照时间、筛选条件、各 Sheet 行数 | `exportJobId` |
...
导出使用任务创建时的 `snapshotAt` 作为数据边界，并尽量在同一个 SQLite 只读快照中读取各 Sheet，确保文章、Event、关系和日志之间可对应。
...
日期输入按 `Asia/Shanghai` 的自然日处理，结束时间使用半开区间，避免边界重复。
