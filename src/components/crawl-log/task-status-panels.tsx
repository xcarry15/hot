...
export function TaskStatusPanels({
  activeTaskView, progressView, sourceCount, loading, latestJobFailure, failedSourcesCount,
  failedArticles, autoRetryArticles, isOperationBusy, onRetryFailedSources, humanQueue, onOpenAttention,
}: TaskStatusPanelsProps) {
  const failedSources = Array.from({ length: failedSourcesCount })
  return (
    <>
        ...
        {!progressView && sourceCount === 0 && !loading && (
        ...
        {(failedSources.length > 0 || failedArticles > 0 || autoRetryArticles > 0) && (
          <div className="flex flex-wrap items-center gap-2 border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
            ...
            {failedSources.length > 0 && <Button size="sm" variant="outline" className="ml-auto h-7 border-amber-300 px-2 text-xs text-amber-900" disabled={isOperationBusy} onClick={() => void onRetryFailedSources()}>一键重试异常源</Button>}
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2 border bg-background px-3 py-2 text-xs">
          ...
          <Button size="sm" variant="outline" className="ml-auto h-7 px-2 text-xs" onClick={onOpenAttention}>打开复核队列</Button>
        </div>
    </>
  )
}
