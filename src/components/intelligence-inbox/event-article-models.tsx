...
import { articlePushStatusLabel, clusterAuditReason, parseEventKeyParts, timeDistanceLabel, timeLabel } from "./utils";
...
function brandDisplay(value: string, representative: string | null): ReactNode {
  ...
  if (!representative || brands.length === 0) return value ? value : "—";
...
function eventKeyDisplay(value: string, representative: string | null): ReactNode {
  ...
  const currentSubjects = currentParts.subjects === "—" ? [] : currentParts.subjects.split("+").map((subject) => subject.trim()).filter(Boolean);
  const representativeSubjects = new Set(representativeParts.subjects === "—" ? [] : representativeParts.subjects.split("+").map(normalizedIdentityToken));
  const sameAction = currentParts.action !== "—" && normalizedIdentityToken(currentParts.action) === normalizedIdentityToken(representativeParts.action);
  const sameObject = currentParts.object !== "—" && normalizedIdentityToken(currentParts.object) === normalizedIdentityToken(representativeParts.object);
  return (
    <>
...
interface CreateEventArticleModelsOptions {
  ...
  moveBrandCandidate: (candidate: EventDetail["brandCandidates"][number]) => Promise<void>;
  moveCurrentArticleToBrandEvent: (candidate: EventDetail["brandCandidates"][number]) => Promise<void>;
  selectArticle: (articleId: string, panel?: "cluster" | "content" | null) => void;
}

export function createEventArticleModels({
  detail, eventDetail, eventMembers, brandCandidates, selectedSplitIds, eventAction,
  ...
  const representativeArticle = eventMembers.find((article) => eventDetail?.representativeArticleId === article.id) ?? null;
  const eventMemberModels: EventArticleRowModel[] = eventMembers.map((article) => {
    ...
    return {
      ...
      sourceStatus: article.source.deleted ? "来源已删除" : article.source.publicEnabled ? "来源可公开" : "来源未开放公开",
      ...
      actions: <>
        ...
        {!representative && (eventDetail?.articleCount ?? 0) > 1 && <Button size="sm" variant="ghost" className="h-7 rounded-none px-2 text-xs text-red-700 hover:bg-red-50 hover:text-red-800" disabled={eventAction !== null} onClick={() => void splitArticle(article.id)}>移出为独立事件</Button>}
      </>,
      tone: "member",
  ...
  const recommendedEventModels: EventArticleRowModel[] = recommendedEventId && recommendedEvent && recommendedArticle
    ? (() => {
        ...
        return [{
          ...
          sourceStatus: article.source.deleted ? "来源已删除" : article.source.publicEnabled ? "来源可公开" : "来源未开放公开",
          publicStatus: recommendedEvent.publicStatus === "published" ? "已公开" : "未公开",
          pushStatus: recommendedEvent.pushedAt ? "已推送" : "未推送",
          actions: <>
            <Button size="sm" variant="ghost" className="h-6 rounded-none px-1 text-[11px] text-slate-600 hover:bg-slate-100 hover:text-slate-900" onClick={() => setComparisonTarget(target)}>查看对比</Button>
            <Button size="sm" variant="ghost" className="h-6 rounded-none px-1 text-[11px] text-sky-700 hover:bg-sky-50 hover:text-sky-800" disabled={eventAction !== null} onClick={handleMoveToRecommendedEvent}>将当前文章移至推荐事件</Button>
          </>,
  ...
  const brandCandidateModels: EventArticleRowModel[] = brandCandidates.map((candidate) => {
    ...
    return {
      ...
      representative: candidate.isEventRepresentative ? <StatusPill tone="accent">其他事件代表文章</StatusPill> : <span className="text-muted-foreground">其他事件成员</span>,
      brand: articleBrands,
      eventKey: candidate.eventKey || "—",
      ...
      sourceStatus: candidate.source.deleted ? "来源已删除" : candidate.source.publicEnabled ? "来源可公开" : "来源未开放公开",
      publicStatus: candidate.publicStatus === "published" ? "已公开" : "未公开",
      pushStatus: candidate.eventPushedAt ? "已推送" : "未推送",
      actions: <>
        <Button size="sm" variant="ghost" className="h-6 rounded-none px-1 text-[11px] text-slate-600 hover:bg-slate-100 hover:text-slate-900" onClick={() => setComparisonTarget(target)}>查看对比</Button>
        <Button size="sm" variant="ghost" className="h-6 rounded-none px-1 text-[11px] text-amber-700 hover:bg-amber-50 hover:text-amber-800" title="将这篇候选文章加入当前文章所属事件" disabled={eventAction !== null} onClick={() => void moveBrandCandidate(candidate)}>并入当前事件</Button>
        <Button size="sm" variant="ghost" className="h-6 rounded-none px-1 text-[11px] text-sky-700 hover:bg-sky-50 hover:text-sky-800" title="将当前文章移入这篇文章所属事件" disabled={eventAction !== null} onClick={() => void moveCurrentArticleToBrandEvent(candidate)}>将当前文章移至该事件</Button>
      </>,
      tone: "brand",
