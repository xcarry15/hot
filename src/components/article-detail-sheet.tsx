...
export default function ArticleDetailSheet({
  ...
  useEffect(() => {
    ...
    request
      .catch((error) => {
        if (!isRequestAborted(error) && !isRequestJsonError(error, 404)) console.error(error);
      })
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full p-0 sm:max-w-lg">
        ...
        <ScrollArea className="h-[calc(100dvh-68px)]">
          {loading ? <div className="space-y-3 p-5"><Skeleton className="h-6 w-3/4" /><Skeleton className="h-20 w-full" /></div> : article ? (
            <div className="space-y-4 p-5">
              ...
              <div className="grid grid-cols-5 border text-center text-xs">
                ...
                <div className="border-r p-2">处理<br /><b>{article.fetchStatus === "fetched" ? "完成" : article.fetchStatus === "failed" ? "失败" : "等待"}</b></div>
                <div className="border-r p-2">聚类<br /><b>{article.clusterStatus}</b></div>
                <div className="border-r p-2">AI<br /><b>{article.aiStatus}</b></div>
          ...
          ) : <div className="p-5 text-sm text-muted-foreground">未找到记录</div>}
        </ScrollArea>
      </SheetContent>
    </Sheet>
  );
