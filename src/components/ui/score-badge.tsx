    ...
    <div className="pointer-events-none absolute left-1/2 -translate-x-1/2 bottom-full mb-1.5 z-50 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
      <div className="bg-white border border-slate-200 rounded-lg shadow-lg px-3 py-2 text-xs whitespace-nowrap">
        <div className="space-y-0.5">
          ...
          {scoreRow('AI 分析置信度', details.aiConfidence)}
...
interface ScoreBadgeProps {
  score: number
  /** 展示风格：pill=带背景圆角徽章(列表) badge=shadcn Badge(详情) text=纯文本色(推送日志) compact=紧凑圆角(文章行) compact-square-wide=详情页宽版方角评分 */
...
export function ScoreBadge({ score, variant = 'pill', details }: ScoreBadgeProps) {
  ...
  const badgeElement = variant === 'text' ? (
    ...
    <span className={`min-w-[2rem] border border-transparent px-1.5 py-0.5 text-center text-xs font-semibold rounded-none tabular-nums shrink-0 ${s.bg} ${s.text} ${hasDetails ? 'cursor-help' : ''}`}>
      {score}
    </span>
  ) : variant === 'compact' ? (
 ...
 * 以单行 flex-wrap 形式展示所有评分细分字段（事件/内容/加权/广告/AI 分析置信度 + 最终分）。
 */
export function ScoreBreakdown({
  ...
  return (
    <div className="flex flex-wrap gap-x-3 text-xs text-muted-foreground">
      ...
      {aiConfidence != null && (
        <span className={`${getScoreStyle(aiConfidence).textOnly}`}>AI 分析置信 {aiConfidence}%</span>
      )}
