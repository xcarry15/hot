...
export default function PublicTab({ settings, setSettings }: Props) {
  ...
  return (
    <div className="space-y-2 pt-2">
        ...
        <CardContent className="space-y-2.5 p-3">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
            <div className="flex items-center gap-2">
              <Label htmlFor="public-min-score" className="whitespace-nowrap text-xs">最低评分</Label>
              <Input
                id="public-min-score"
                type="number"
                min={0}
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="public-min-relevance" className="whitespace-nowrap text-xs">最低相关度</Label>
              <Input
                ...
                onChange={(event) => setSettings((prev) => ({ ...prev, public_min_relevance: event.target.value }))}
                className="h-8 w-20 text-xs"
              />
            </div>
      ...
      <Card className="py-0">
        <CardHeader className="border-b px-3 py-2"><CardTitle className="text-sm">数据源公开开关</CardTitle></CardHeader>
        <CardContent className="space-y-2 p-3">
          ...
          {loadingSources ? <Skeleton className="h-10 w-full" /> : sources.length === 0 ? <p className="py-3 text-xs text-muted-foreground">暂无数据源</p> : (
            <div className="grid border sm:grid-cols-2 xl:grid-cols-3">
              {sources.map((source) => <div key={source.id} className="flex items-center justify-between gap-3 border-b border-r px-2.5 py-1.5"><div className="min-w-0"><p className="truncate text-xs font-medium">{source.name}</p><p className="text-[10px] text-muted-foreground">{source.articleCount} 篇文章</p></div><Switch className="scale-75" checked={source.publicEnabled} onCheckedChange={() => toggleSource(source)} aria-label={`${source.name}公开开关`} /></div>)}
            </div>
          )}
        </CardContent>
      </Card>
