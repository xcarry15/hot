...
- event_subjects：1-3 个直接参与该事实的主体，使用正式名称或稳定简称；不写媒体、记者、地点、产品、IP 或只被顺带提及的品牌
- event_action：只写一个原子动作，优先使用以下稳定短语：计划开店、正式开店、计划关店、关闭门店、任命高管、高管离任、增持股份、减持股份、发布业绩、融资上市、完成收购、启动合作、上线功能、发布产品、价格上涨、价格下调、监管处置、争议维权、捐赠救援、获得奖项
- event_action 必须保留计划/正式/完成等阶段；禁止“布局、升级、发力、加码、推进、深化、探索、调整战略、应对竞争、打造模式”等空泛动作，也禁止使用“并、同时、以及”串联多个动作
- event_object：只写区分该事件的最小事实单元，格式优先为“对象 + 结果/数量/地点/季度”；保留门店名称、城市、批次、季度、金额、数量、产品名等辨识信息，最多 48 个汉字，不要复制摘要或连续罗列多个结果
- 同一事件的不同报道必须尽量输出相同的主体、原子动作和关键限定；如果只有行业方向或泛泛观点，没有可定位的具体事实，降低 event_key_confidence
...
- 不自行拼接 event_key，程序会按“主体/原子动作/关键事项”统一生成`;
...
export function buildStep2Prompt(
  ...
  return [
    ...
    eventIdentityBlock,
    '',
    '事件身份硬约束（不可被评判块覆盖）：event_action 只能是一个原子动作，禁止战略概括和多动作串联；event_object 不超过 48 个汉字，只保留能区分事件的对象、结果、地点、季度、金额或数量；身份宽泛或缺少限定时 event_key_confidence 不得高于 60。',
    '',
    '输出 JSON：',
    '{',
    '  "is_ad": <true/false>,',
