...
export function buildStep2Prompt(
  ...
  return [
    ...
    eventIdentityBlock,
    '',
    '事件身份硬约束（不可被评判块覆盖）：event_subjects 每项只写一个名称词，event_action 只写一个动作词，event_object 只写一个辨识词或短语；三者都禁止完整句、并列词和解释文字，event_subjects 单项不超过 16 个汉字，event_action 不超过 8 个字符，event_object 不超过 16 个汉字；身份宽泛或缺少限定时 event_key_confidence 不得高于 60。',
    '',
    '输出 JSON：',
    '{',
    '  "is_ad": <true/false>,',
