...
export const DEFAULT_SYSTEM_PROMPT = `你是一个深耕连锁消费行业的资深分析师,你擅长第一性原理思考，分析信息的本质。`;
...
- 如果是：is_ad=true
- 否则：is_ad=false
- ad_probability：0-19低，20-39可疑，40-100高；≥40 时 is_ad=true。`;
...
只评估“文章内容本身是否值得读者优先关注”，不看文章文笔。重点锚定【热门品牌】【人事变动】【门店规模】【融资/IPO】四类高优事件。无视文章写作质量。
...
// 设计目标:精炼、核心,每条 30 字以内的高密度信息。
export const DEFAULT_BLOCK_KEY_POINTS = `【key_points｜核心要点，每条≤30字】
-提取原文核心要点，数据确保从原文取值。
- 每条格式为“主体 + 动作/变化 + 数据/结果"
- 同一事实不拆分，不重复标题，不写评价、动机或空泛趋势
- 文章只有1个核心要点时，只输出 1 条，最多输出 5 条`;

// 洞察块：产出 summary（111~222 字，一针见血、直指本质）
// 设计目标:让连锁品牌企业的数据分析师读完能快速把握竞品动态、品牌战略走向和行业信号。
export const DEFAULT_BLOCK_SUMMARY = `【summary｜111~222字】
基于第一性原理，客观点评文章内容：抓出文章中最硬的事实，点破背后的真实算盘，客观说清它会伤到谁、利好谁，预判后续发展。
表达风格：贴吧老哥嘴臭风格，犀利、毒舌，一针见血。

`;

// 事件身份块：产出三段式事件键原料，由程序确定性生成最终 eventKey。
// 设计目标：不同媒体对同一件事改写标题时，仍能稳定提取相同的主体、行为和具体事项。
export const DEFAULT_BLOCK_EVENT_IDENTITY = `【事件身份词｜4 个字以内】
...
- 无法确认主事实时三项留空、event_key_confidence=0；身份宽泛或事项无区分度时≤60；不要编造`;
