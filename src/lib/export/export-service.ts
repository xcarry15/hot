...
import type { ExportJob, ExportJobStatus } from '@prisma/client';
...
function assertSafeStorageKey(storageKey: string): void {
  if (!/^[0-9a-f-]{36}\.xlsx$/i.test(storageKey)) throw new Error('Invalid export storage key');
}
...
async function processExportJob(job: ExportJob): Promise<void> {
  ...
  try {
    ...
    await import('node:fs/promises').then(({ writeFile }) => writeFile(storagePath(temporaryKey), result.buffer));
