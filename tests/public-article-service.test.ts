...
vi.mock('@/lib/db', () => ({
  db: {
    event: { findMany: mocks.eventFindMany, findFirst: mocks.eventFindFirst },
...
describe('public-article-service Event 门禁', () => {
  ...
  it('一个 Event 只输出一张卡片并携带来源数', async () => {
    mocks.eventFindMany.mockResolvedValue([eventRow('e1', '2026-07-15T01:00:00Z', 3)]);
  ...
  it('详情使用 Event.id，并列出全部来源', async () => {
    mocks.eventFindMany.mockResolvedValue([eventRow('e1', '2026-07-15T01:00:00Z', 2)]);
    mocks.articleFindMany.mockResolvedValue([
