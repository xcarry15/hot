...
- AI 偶尔只返回评分/摘要而漏掉事件身份时，解析器兼容常见字段命名，并仅追加一次轻量身份修复；修复仍无法定位具体事件的文章按正常跳过处理，不进入技术失败重试队列。
