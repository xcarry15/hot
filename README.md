...
收件箱支持当前页多选后批量归类、重新分析和重新抓取；处理完成会自动定位下一篇，`↑/↓` 可切换文章，`I/G/N` 分别归类为重要/一般/无关。筛选、页码和当前位置会在浏览器本地记忆。抓取记录按数据源聚合异常，可批量重试失败源，并将常见网络、超时和熔断错误翻译为可操作提示。设置页默认展示常用入口，高级 AI、提示词、推送、账户和数据清理设置可展开；修改评分权重或公开规则后会提示重算/快照同步数量，并可预览公开与推送结果。公开文章支持关键词、数据源和日期筛选；详情页支持相关推荐、分享和复制链接，加载失败时可直接重试。
