...
情报收件箱是全量文章人工校准工作台：保持左侧紧凑表格与右侧详情布局，默认展示全部文章（含抓取中、抓取失败和 AI 未完成记录），支持分页、搜索、来源/评分/状态筛选、人工修改评分构成与内容判断、恢复 AI 原值、归类、公开策略、批量重新分析和重新抓取；总分由服务端统一重算，AI 置信度只读。人工覆盖保存在文章字段上，后续 AI 重分析保留覆盖值，并可按人工反馈生成可解释调优建议。处理完成会自动定位下一篇，`↑/↓` 可切换文章，`I/G/N` 分别归类为重要/一般/无关。筛选、页码和当前位置会在浏览器本地记忆。抓取记录按数据源聚合异常，可批量重试失败源，并将常见网络、超时和熔断错误翻译为可操作提示。设置页默认展示常用入口，高级 AI、提示词、推送、账户和数据清理设置可展开；修改评分权重或公开规则后会提示重算/快照同步数量，并可预览公开与推送结果。公开文章支持关键词、数据源和日期筛选；详情页支持相关推荐、分享和复制链接，加载失败时可直接重试。
